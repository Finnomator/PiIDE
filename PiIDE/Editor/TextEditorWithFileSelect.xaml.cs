...
public partial class TextEditorWithFileSelect {
    ...
    private void AddFile(string filePath, bool onBoard = false, int atIndex = -1) {
        ...
        textEditor.SavedFile += (_, _) => UpdatePylintMessages(textEditor);
        textEditor.StartedPythonExecution += (_, _) => OutputTabControl.SelectedIndex = 2;

        tabItem.CloseTabClick += (_, path) => {
    ...
    public void AddExistingDirectory(string directory) {
        ...
        LocalExplorer.FileRenamed += (_, oldPath, newPath) => {
    ...
    public void OpenBoardDirectory() {
        ...
        BoardExplorer.FileDeleted += (_, filePath) => {
            CloseFile(filePath);
        };

        BoardExplorer.FileRenamed += (_, oldPath, newPath) => {
            if (File.Exists(newPath)) {
                OpenRenamedFile(oldPath, newPath);
            }
        };
    ...
    private void GoToPylintMessage(PylintMessage pylintMessage) => GoTo(pylintMessage.Path, pylintMessage.Line - 1, pylintMessage.Column);

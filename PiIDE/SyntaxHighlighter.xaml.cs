...
namespace PiIDE {
    ...
    public partial class SyntaxHighlighter : UserControl {
        ...
        private List<TextBlock> OldChildren = new();
        private List<TextBlock> NewChildren = new();
        ...
        public async Task HighglightTextAsync(string text, string filePath) {
            ...
            await AddHighlightedJediWordsToChildrenAsync(filePath);
            ...
            for (int i = 0; i < max; ++i) {
                ...
                if (i < NewChildren.Count && i < OldChildren.Count) {
                    if (!NewChildren[i].Equals(OldChildren[i])) {
        ...
        private void AddHighlightedKeywordsToChildren(string text) {
            ...
            foreach (Match match in FindKeywords(text)) {
                ...
                NewChildren.Add(new() {
                    Text = keyword,
                    Margin = new((indexPoint.X + 0.5) * FontSizes.Width, indexPoint.Y * FontSizes.Height, 0, 0),
        ...
        private async Task AddHighlightedJediWordsToChildrenAsync(string filePath) {

            JediSyntaxHighlightedWord[] jediSyntaxHighlightedWords = await JediSyntaxHighlighterWraper.GetHighlightedWordsAsync(filePath);
            ...
            for (int i = 0; i < jediSyntaxHighlightedWords.Length; ++i) {
                ...
                NewChildren.Add(new() {
                    Text = name,
                    Margin = new((col + 0.5) * FontSizes.Width, (row - 1) * FontSizes.Height, 0, 0),
        ...
        public IEnumerable<Match> FindKeywords(string text) {
